		...
		for (User user : listUser) {
			if (email.trim().equals(user.getEmail())) {
				...
				String body = "<div>\r\n" + "<h3>Your OTP verification code is: <span style=\"color:#119744; font-weight: bold;\">"
						+ random_otp + "</span></h3>\r\n" + "</div>";
				sendMailService.queue(email, "Forgot password?", body);

				model.addAttribute("email", email);
				model.addAttribute("message", "OTP verification code has been sent to email : " + user.getEmail() + " , please check your inbox!");
